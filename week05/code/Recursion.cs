...
public static class Recursion
{
    ...
    public static decimal CountWaysToClimb(int s, Dictionary<int, decimal>? remember = null)
    {
        ...
        // Use memoization to store the result for future calls
...
public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<(int, int)>? currPath = null)
{
    ...
    // Create new path to avoid mutation across recursion branches
    ...
    SolveMaze(results, maze, x + 1, y, newPath); // right
    SolveMaze(results, maze, x - 1, y, newPath); // left
    SolveMaze(results, maze, x, y + 1, newPath); // down
    SolveMaze(results, maze, x, y - 1, newPath); // up
}
